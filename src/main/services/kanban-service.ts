...
export class KanbanService {
  ...
  createItem(title: string, description: string, column: KanbanColumnType, linkedIssueKey?: string): KanbanItem {
    ...
    const maxPositionRow = this.db.prepare(`
    ...
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(title, description, column, position, linkedIssueKey || null, now, now);

    const item = this.db.prepare('SELECT * FROM KanbanItems WHERE Id = ?').get(result.lastInsertRowid) as any;
    return this.mapRowToItem(item);
  }

  updateItem(id: number, title: string, description: string, linkedIssueKey?: string): void {
    const now = new Date().toISOString();
    this.db.prepare(`
      UPDATE KanbanItems 
  ...
  moveItem(id: number, newColumn: KanbanColumnType, newPosition: number): void {
    const now = new Date().toISOString();
    
    // Get current item
    const item = this.db.prepare('SELECT * FROM KanbanItems WHERE Id = ?').get(id) as any;
    if (!item) return;
