...
> = ({ onIssueDoubleClick }) => {
    ...
    const loadRecords = async () => {
        try {
            const data =
                await window.electronAPI.getYesterdayTimeTrackingRecords()
            setRecords(data)
    ...
    const aggregateRecords = (): CompactDisplayRecord[] => {
        ...
        records.forEach((record) => {
            ...
            if (aggregated.has(record.issueKey)) {
                ...
                aggregated.set(record.issueKey, {
                    totalMs: durationMs,
                    count: 1,
                })
            }
    ...
    const formatLogCount = (row: CompactDisplayRecord) =>
        row.logCount.toString()
            ...
            {isCompactMode ? (
                <DataGrid
                    ...
                    onRowDoubleClick={(row) =>
                        onIssueDoubleClick?.(row.issueKey)
                    }
                />
            ) : (
                <DataGrid
