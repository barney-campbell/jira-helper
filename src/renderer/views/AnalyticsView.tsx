  ...
  if (!insights || dailyStats.length === 0) {
    return (
      <AnalyticsContainer>
        <h1>Personal Analytics</h1>
        <EmptyState>
          No time tracking data available yet. Start tracking time on issues to
          see your analytics!
        </EmptyState>
      </AnalyticsContainer>
    );
      ...
      <Grid>
        ...
        <Card>
          ...
          {insights.longestSession ? (
            <>
              <StatValue>
                {formatDuration(insights.longestSession.duration)}
              </StatValue>
              <StatLabel>
                {insights.longestSession.issueKey} on{" "}
                {formatDate(new Date(insights.longestSession.date))}
              </StatLabel>
            </>
          ) : (
        ...
        <ChartContainer>
          <BarChart>
            {hourlyStats.map((hour, index) => {
              ...
              return (
                <Bar key={index} $height={heightPercent}>
                  {heightPercent > BAR_VALUE_VISIBILITY_THRESHOLD && (
                    <BarValue>{formatDuration(hour.totalSeconds)}</BarValue>
                  )}
