...
import type { ProductivityInsights, DailyStats, HourlyStats, IssueStats } from '../../common/types';
  ...
  if (!insights || dailyStats.length === 0) {
    return (
      <AnalyticsContainer>
        <h1>Personal Analytics</h1>
        <EmptyState>
          No time tracking data available yet. Start tracking time on issues to see your analytics!
        </EmptyState>
      </AnalyticsContainer>
    );
  }

  const weeklyChange = insights.totalTimeLastWeek > 0
  ...
  return (
    <AnalyticsContainer>
      ...
      <Grid>
        <Card>
          <h2>This Week</h2>
          <StatValue>{formatDuration(insights.totalTimeThisWeek)}</StatValue>
        ...
        <Card>
          <h2>Daily Average</h2>
          <StatValue>{formatDuration(insights.dailyAverage)}</StatValue>
        ...
        <Card>
          ...
          {insights.longestSession ? (
            <>
              <StatValue>{formatDuration(insights.longestSession.duration)}</StatValue>
              <StatLabel>
                {insights.longestSession.issueKey} on {formatDate(new Date(insights.longestSession.date))}
              </StatLabel>
            </>
          ) : (
      ...
      <Grid>
        <Card>
          ...
          {insights.mostProductiveHours.length > 0 ? (
            <>
              <InsightText>
                You tend to be most productive during these hours:
              </InsightText>
              <List>
                {insights.mostProductiveHours.map(hour => (
                  <ListItem key={hour}>
                    <ItemLabel>{hour}:00 - {hour + 1}:00</ItemLabel>
                  </ListItem>
                ))}
              </List>
            </>
          ) : (
            <InsightText>Not enough data to determine productive hours</InsightText>
          )}
        </Card>
            ...
            <List>
              {insights.mostWorkedIssues.map(issue => (
                <ListItem key={issue.issueKey}>
                  ...
                  <ItemValue>{formatDuration(issue.totalSeconds)}</ItemValue>
                </ListItem>
              ))}
            </List>
          ) : (
      ...
      <Card>
        ...
        <ChartContainer>
          <BarChart>
            {hourlyStats.map((hour, index) => {
              const heightPercent = (hour.totalSeconds / maxHourlySeconds) * 100;
        ...
        <InsightText>
          This chart shows your total time tracked by hour across all days.
        </InsightText>
      </Card>
    </AnalyticsContainer>
  );
